...
export function buildLoomEmbedHtml(match: LoomEmbedMatch): string {
  ...
  return [
    ...
    '<button type="button" data-loom-expand contenteditable="false" aria-label="Expand Loom player" title="Expand Loom player" class="inline-flex h-7 w-7 items-center justify-center rounded-md border border-foreground/20 text-foreground/80 transition-colors hover:border-foreground/45 hover:text-foreground focus:outline-none focus:ring-1 focus:ring-foreground/45">',
